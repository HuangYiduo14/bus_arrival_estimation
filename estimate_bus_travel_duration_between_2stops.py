...
for line_id in linenum_list:
    ...
    new_start_end['count'] = new_start_end['count'] * (1. * new_start_end['station']==station2)
    new_start_end['board_count'] = new_start_end['count'] * new_start_end['is_start']
    new_start_end['alight_count'] = new_start_end['count'] * (1.- new_start_end['is_start'])
    ...
    result_diff0 = new_start_end.groupby(level=0).agg({'diff_max': max_min, 'count': 'sum','max_pivot':'mean','board_count':'sum','alight_count':'sum'}).dropna()
    result_diff0['line_id']=line_id
    result_diff = pd.concat([result_diff,result_diff0])

result_diff.to_csv('result_{0}_{1}_diff_2.csv'.format(stop_pair[0],stop_pair[1]))
